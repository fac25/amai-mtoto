...
function weeksUntilMappedToPregnancyWeek(weeksUntil) {
  let PregnancyWeek = 41 - weeksUntil;
...
function getArticleByTopic(articles, topicName) {
  return articles.filter((article) => article.topic.includes(topicName));
}
function capitalizeFirstLetter(string) {
  return string.charAt(0).toUpperCase() + string.slice(1);
}

function dateInFourtyOneWeeksTime() {
  let today = new Date().getTime();
  let dateInFourtyOneWeeksTime = convertMillisecondsToDateStringFormat(
    today + 41 * 604800000
  );
  //console.log(dateInFourtyOneWeeksTime);
  return dateInFourtyOneWeeksTime;
}
